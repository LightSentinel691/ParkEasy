...
export default function Vehicles() {
  ...
  const now = new Date();
  const formattedTimeOut = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <div className="p-8 bg-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Checked In */}
        <div>
          <h2 className="text-xl font-semibold mb-2">Vehicles Checked In</h2>
        ...
        {/* Checked Out */}
        <div>
          <h2 className="text-xl font-semibold mb-2">Vehicles Checked Out</h2>
            ...
            <table className="w-full table-auto text-left">
              ...
              <tbody>
                {checkedOut.map((v) => (
                  <tr
                    key={v.id}
                    onClick={() => setSelectedVehicle({ ...v, type: "out" })}
                    className="cursor-pointer hover:bg-gray-50"
                  >
                    <td className="p-3">{v.vehiclePlate}</td>
                    <td className="p-3">{v.ownerName}</td>
      ...
      {selectedVehicle && (
        <Modal onClose={() => setSelectedVehicle(null)}>
          {selectedVehicle.type === "out" ? (
            <div>
              <h2 className="text-xl font-semibold mb-2">Checked Out Details</h2>
              <p><strong>Plate:</strong> {selectedVehicle.vehiclePlate}</p>
              <p><strong>Time In:</strong> {selectedVehicle.timeIn}</p>
              <p><strong>Time Out:</strong> {selectedVehicle.timeOut}</p>
              <p className="mt-2"><strong>Amount Paid:</strong> ${calculateFee(selectedVehicle.clockIn, selectedVehicle.clockOut)}</p>
            </div>
          ) : (
            <div>
    <h2 className="text-xl font-semibold mb-2">Checked In Details</h2>
    <p><strong>Plate:</strong> {selectedVehicle.vehiclePlate}</p>
    <p><strong>Time In:</strong> {selectedVehicle.timeIn}</p>
    <p><strong>Current Time:</strong> {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
    <p className="mt-2"><strong>Estimated Fee:</strong> ${calculateFee(selectedVehicle.clockIn, `${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}` )}</p>
